...
Deno.serve(async (req) => {
  ...
  const { email, role, token, expiresAt, inviterName } = await req.json();

  const roleLabel = role === 'professional' ? 'Profissional' : 'Paciente';
  const inviteUrl = `${req.headers.get('origin') || 'https://app.base44.com'}/UserInvite?token=${token}`;
  ...
  await base44.integrations.Core.SendEmail({
    to: email,
    subject: `Convite para acessar a Clínica Espaço Saúde - ${roleLabel}`,
    body: emailBody
  });
